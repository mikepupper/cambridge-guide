...
for i,(cid,title,short,desc) in enumerate(categories):
 ...
 for j,p in enumerate(group):
  ...
  cards.append(f'''<article class="card">
   ...
   <div class="card-body"><p class="card-meta">{short}</p><h3>{H(p['name'])}</h3><p class="description">{H(p['description'])}</p>{('<p class="practical">'+H(p['practical'])+'</p>') if p.get('practical') else ''}<p class="address">{H(p['address'])}</p></div>
...
<body id="arriba"><a class="skip" href="#guia">Saltar a los lugares</a><header class="masthead"><a class="wordmark" href="#arriba">Cambridge<span>GUÍA DE BOLSILLO</span></a></header>
...
<nav class="category-nav" aria-label="Categorías de lugares"><div class="nav-bar"><div class="nav-inner">{nav}</div><button type="button" class="surprise-btn" id="surprise-btn">Sorpréndeme</button></div></nav>
