...
for i,(cid,title,short,desc) in enumerate(categories):
 ...
 for j,p in enumerate(group):
  ...
  cards.append(f'''<article class="card">
   ...
   <div class="card-body"><p class="card-meta">{short} · Cambridge</p><h3>{H(p['name'])}</h3><p class="description">{H(p['description'])}</p>{('<p class="practical">'+H(p['practical'])+'</p>') if p.get('practical') else ''}<p class="address">{H(p['address'])}</p></div>
...
page=f'''<!doctype html>
<html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="theme-color" content="#264f43"><meta name="description" content="Guía de bolsillo de Cambridge: {len(places)} lugares a pie, entre clásicos y rincones locales. Parques, cultura, miradores y cafés independientes con fotografías y mapas."><title>Cambridge · A tu aire</title><link rel="stylesheet" href="styles.css"><script src="guide.js" defer></script></head>
<body id="arriba"><a class="skip" href="#guia">Saltar a los lugares</a><header class="masthead"><a class="wordmark" href="#arriba">Cambridge<span>GUÍA DE BOLSILLO</span></a><span class="edition">Inglaterra<br>A pie · A tu ritmo</span></header>
<div class="intro"><div><p class="eyebrow">Para vivir Cambridge</p><h1>Cambridge,<br><em>a tu aire.</em></h1></div><div class="intro-copy"><p>Una guía para tus tres semanas: elige un rincón según lo que te apetezca hoy, sin horarios ni días asignados.</p><div class="stats"><span><strong>{len(places)}</strong> lugares</span><span><strong>6</strong> categorías</span><span><strong>{sum(p['local'] for p in places)}</strong> con espíritu local</span></div></div></div>
<nav class="category-nav" aria-label="Categorías de lugares"><div class="nav-inner">{nav}</div></nav><main class="guide" id="guia">{''.join(sections)}
